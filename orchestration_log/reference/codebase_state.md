...
- **Workspace version** (all three crates share it via `version.workspace = true`): **0.2.3**
- Latest tags on `main`: `filmdrop-web-v0.2.3`, `filmdrop-cli-v0.2.3`, `filmdrop-core-v0.2.3` (released 2026-06-14)
...
- Docker image: 2-stage build using cargo-binstall to download prebuilt binary (no Rust toolchain); final image is `debian:bookworm-slim` with non-root user (132 MB)
- `[package.metadata.binstall]` in both filmdrop-cli and filmdrop-web for `cargo binstall` support, with Windows `.zip` overrides
...
1. **Test coverage** — zero tests in the workspace; top priority carried from two sessions
2. Pick up deferred items from `deferred_items.md` if now scheduled
3. Run `just test` before any code change to confirm baseline
4. If working on the image pipeline, read `filmdrop-cli/src/image_processor.rs` and the in-memory buffer pattern before touching upload logic
5. If working on Docker, test with `docker build -t filmdrop-web . && docker run --rm -e GALLERY_BUCKET=test filmdrop-web`
